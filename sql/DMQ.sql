...
FROM genres;
SELECT singerName AS name,
    homeCity AS city,
    homeState AS state
FROM singers;
SELECT albumName as name,
    singerName as singer,
    albumReleaseDate AS releaseDate
FROM albums
...
WHERE genreID = "3";
UPDATE albums
SET albumName = "My Beautiful Dark Twisted Fantasy",
    albumReleaseDate = 2010 -11 -22
WHERE albumID = "1";
UPDATE singers
SET singerName = "Kanye West",
    homeCity = "Atlanta",
    homeState = "CA"
WHERE singerID = "1";
UPDATE songs
...
VALUES (
        ...
        (
            SELECT albumID
            from albums
            where albumName = "How Do You Feel Now?"
        ),
        (
            SELECT genreID
            FROM genres
            WHERE genreName = "Indie Rock"
        )
    );
